...
function setCache(key, value) {
  if (operatorCache.size >= MAX_CACHE) {
    const firstKey = operatorCache.keys().next().value;
    operatorCache.delete(firstKey);
  }
  operatorCache.set(key, value);
}

export async function detectOperator(phoneNumber) {
  const normalized = String(phoneNumber).replace(/\D/g, "");

  if (operatorCache.has(normalized)) {
    return operatorCache.get(normalized);
  }

  try {
    const res = await fetch(
      `https://api.asuma.my.id/v1/tools/detect-operator?phoneNumber=${encodeURIComponent(normalized)}&apikey=ditss`
    );

    if (res.ok) {
      const data = await res.json();
      if (data.status && data.result?.operator) {
        setCache(normalized, data.result.operator);
        return data.result.operator;
      }
    }
  } catch {}

  const localResult = detectOperatorLocal(normalized);
  setCache(normalized, localResult);
  return localResult;
}
