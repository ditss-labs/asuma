...
export async function detectOperator(phoneNumber) {
  ...
  if (operatorCache[normalizedNumber]) {
    return operatorCache[normalizedNumber];
  }
  
  const baseUrl = 'https://api.asuma.my.id';
  const endpoints = [
    `${baseUrl}/v1/tools/detect-operator?phoneNumber=${encodeURIComponent(phoneNumber)}&apikey=ditss`,
    ...
    for (const url of endpoints) {
      ...
      if (res.ok) {
        ...
        if (data.status && data.result && data.result.operator) {
          operatorCache[normalizedNumber] = data.result.operator;
