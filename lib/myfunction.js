...
if (typeof m.sender === 'string' && m.sender.includes(':')) {
  m.sender = jidNormalizedUser(m.sender)
        ...
        m.jid = global.db?.users?.[m.sender]?.id || m.sender
        m.senderTest = Ditss.decodeJid(m.fromMe ? Ditss.user.id : m.participant || m.key.participant || m.chat || '');
...
if (m.message) {
    ...
    let quoted = m.quoted = m.msg && m.msg.contextInfo?.quotedMessage || null;
    m.mentionedJid = m.msg && m.msg.contextInfo?.mentionedJid || [];
    ...
    if (m.quoted) {
        ...
        m.quoted.mtype = type;
        m.quoted.id = m.msg && m.msg.contextInfo?.stanzaId;
        m.quoted.chat = (m.msg && m.msg.contextInfo?.remoteJid) || m.chat;
        m.quoted.isBaileys = m.quoted.id?.startsWith('BAE5') && m.quoted.id.length === 16;
        m.quoted.sender = Ditss.decodeJid(m.msg && m.msg.contextInfo?.participant);
        ...
        m.quoted.mentionedJid = m.msg && m.msg.contextInfo?.mentionedJid || [];
