...
function loadUser(id, pushName = "Unknown") {
  ...
  if (!data.id) data.id = id;
  if (!data.name || data.name === "") data.name = pushName;
  if (!data.joinDate || data.joinDate === "") data.joinDate = new Date().toISOString();
...
// Default user jika file baru dibuat
const defaultUser = {
  name: "",
  vip: false,
  ban: false,
...
function makeUserProxy(data, id) {
  return new Proxy(data, {
    set(target, prop, value) {
      target[prop] = value;
      saveUser(id, target); // langsung save tiap perubahan property
      return true;
    }
  });
}
