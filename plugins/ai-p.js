...
let aiImageChat = async (m, { conn: Ditss, usedPrefix, command, args }) => {
  if (!args[0])
  ...
  try {
    ...
    if (result.imgeGenerate?.length) {
      ...
      await Ditss.sendMessage(m.chat, { image: { url: imageUrl }, caption }, { quoted: m });
      return;
    }
    ...
    await Ditss.sendMessage(m.chat, { text: fullResponse.trim() }, { quoted: m });
