...
let veo2 = async (m, { conn: Ditss, text }) => {
  ...
  try {
    ...
    if (videoBuffer.length === 0) {
      return m.reply('❌ Video kosong atau tidak valid')
    }
    
    if (videoBuffer.length > 50 * 1024 * 1024) {
      return m.reply(`❌ Video terlalu besar (${(videoBuffer.length / (1024 * 1024)).toFixed(2)} MB)\n\nSilakan download manual:\n${video_url}`)
    }
    
    await Ditss.sendMessage(m.chat, {
      video: videoBuffer,
      caption: `🎬 *VE02 AI Video*\n\n📝 *Prompt:* ${prompt}\n⏱️ *Waktu proses:* ${data.responseTime}\n\nGenerated by @${global.info?.namabot || 'Bot'}`
    }, { quoted: m })
    ...
    if (error.code === 'ECONNABORTED') {
      m.reply('❌ Timeout: Server terlalu lama merespon (lebih dari 2 menit)')
    } else if (error.response?.status === 404) {
      m.reply('❌ Video tidak ditemukan atau sudah dihapus dari server')
    } else if (error.response) {
      m.reply(`❌ Error ${error.response.status}: Gagal membuat video`)
    } else if (error.code === 'ENOTFOUND') {
      m.reply('❌ Server API tidak dapat diakses')
    } else {
