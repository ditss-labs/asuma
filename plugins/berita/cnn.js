...
let cnn = async (m, { conn: Ditss, text, usedPrefix, command }) => {
  const types = [
    "nasional",
    "internasional",
    "ekonomi",
  ...
  if (!text) {
    let buttons = [
      {
        buttonId: "cnn",
        buttonText: { displayText: "📰 Pilih Kategori CNN" },
        type: 4,
        nativeFlowInfo: {
          name: "single_select",
          paramsJson: JSON.stringify({
            title: "📊 Pilih Kategori CNN Indonesia",
            sections: [
              {
  ...
  try {
    ...
    const apiUrl = `${global.api.domain}/v1/berita/cnn-news?apikey=${global.api.key}&category=${category}`
        ...
        message: {
          ...
          interactiveMessage: proto.Message.InteractiveMessage.fromObject({
            body: proto.Message.InteractiveMessage.Body.create({
              text: `📰 *${category.toUpperCase()} - CNN Indonesia*`
            }),
...
cnn.help = ['cnn <kategori>']
cnn.tags = ['news']
cnn.command = ['cnn', 'cnnnews']
cnn.limit = 2
