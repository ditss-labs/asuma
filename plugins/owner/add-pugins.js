...
let addPlugins = async (m, { conn: Ditss, args, usedPrefix }) => {
    const fullArgs = args.join(' ');
    const commaIndex = fullArgs.indexOf(',');

    if (commaIndex === -1) {
        return Ditss.sendMessage(m.chat, { text: `❌ Format salah! Gunakan:\n${usedPrefix}addplugins namafile, <kode plugin>` }, { quoted: m });
    }

    const pluginNameInput = fullArgs.substring(0, commaIndex).trim();

    if (!pluginNameInput) {
        return Ditss.sendMessage(m.chat, { text: '❌ Nama file tidak boleh kosong.' }, { quoted: m });
    }

    if (!pluginNameInput.match(/^[a-zA-Z0-9_-]+$/)) {
        return Ditss.sendMessage(m.chat, { text: '❌ Nama file hanya boleh huruf, angka, _ atau -.' }, { quoted: m });
    }
    
    const code = fullArgs.substring(commaIndex + 1).trim();
    
    if (!code) {
        return Ditss.sendMessage(m.chat, { text: '❌ Kode plugin tidak boleh kosong.' }, { quoted: m });
    }
    ...
    try {
        ...
        Ditss.sendMessage(m.chat, { text: `✅ Plugin berhasil ditambahkan!\nNama file: ${pluginName}\nRestart bot untuk mengaktifkan.` }, { quoted: m });
    } catch (e) {
        console.error(e);
        Ditss.sendMessage(m.chat, { text: '❌ Gagal menambahkan plugin.' }, { quoted: m });
    }
}
