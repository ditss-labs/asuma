...
async function groupStatus(Ditss, jid, content) {
    ...
    await Ditss.relayMessage(jid, m.message, { messageId: m.key.id });
    return m;
}

let handler = async (m, { conn: Ditss, usedPrefix, command }) => {
    try {
        ...
        await groupStatus(Ditss, m.chat, payload);
