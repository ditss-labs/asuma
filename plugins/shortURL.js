...
let tinyurl = async (m, { conn: Ditss, usedPrefix, args, command }) => {
  if (!args[0]) {
    return m.reply(
      `⚠️ Masukkan URL yang mau di-shortener!\n\nContoh:\n${usedPrefix + command} api-ditss.vercel.app`
    )
  }
  ...
  try {
    ...
    const apiUrl = `${api.ditss}/tools/tinyurl?url=${encodeURIComponent(url)}`
